...
func TestExecuteUpdateFeedbacks(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                 "success",
			feedbacks:            &FeedbackItems{Items: []string{"feedback 1", "feedback 2"}},
			mockResult:           []sqlc.Feedback{{ID: 1, Content: "feedback 1"}, {ID: 2, Content: "feedback 2"}},
			createFeedbacksError: nil,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(*testing.T) {
			...
			msgs := executeFeedbacksCommands(
				ctx, feedbacksRepo, mockTelegramID, tt.feedbacks, mockLogger)
...
func TestProcessFeedbackListLambda(t *testing.T) {
	feedbacksRepo := new(mockFeedbacksRepo)
	mockLogger := slog.New(slog.NewTextHandler(io.Discard, nil))
	mockAgentState := &AgentState{TelegramUserID: 123321}
	tests := []struct {
		name      string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(*testing.T) {
			...
			ctx := context.Background()
			message := listFeedbackItems(ctx, mockAgentState, feedbacksRepo, mockLogger)
			if tt.mockError == nil {
...
func TestParseFeedbackItems(t *testing.T) {
	mockLogger := slog.New(slog.NewTextHandler(io.Discard, nil))
	ctx := context.Background()
	...
	tests := []struct {
		name       string
		rawMessage string
		expected   *FeedbackItems
	}{
		{
			name:       "valid json",
			rawMessage: `{"items":["feedback 1","feedback 2"]}`,
			expected: &FeedbackItems{
				Items: []string{"feedback 1", "feedback 2"},
			},
		},
		{
			name:       "invalid json returns empty items",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := parseFeedbackItems(ctx, tt.rawMessage, mockLogger)
